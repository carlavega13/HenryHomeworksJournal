...
// Tu código acá: 
//?     le agrego a una etiqueta html mi nombre
document.querySelector("#createdBy").innerHTML =
  document.querySelector("#createdBy").innerHTML + " Carla Vega";
//?-----------------------------------------------------------------------
...
// Tu código acá:
//?       le agrego un metodo a la clase ToDo
...
//? buildToDo recibe un toDo y un index , este es mi componente card
...
function displayToDos() {
  ...
  for (let i = 0; i < toDoShells.length; i++) {
    toDoContainer.appendChild(toDoShells[i]);
  }
}

// La función 'addToDo' agregará un nuevo ToDo al array 'toDoItems'
// [NOTA: Algunas cuestiones a tener en cuenta sobre el elemento 'input' de HTML (Ya que 'toDoInput' es un input)
// Toddos los elementos input tienen una propiedad llamada 'value' que nos permite acceder al texto que se encuentre
...
function addToDo() {
  // Tu código acá:
  if (document.querySelector("#toDoInput").value.trim() === "") return;
  const toDo = new ToDo(document.querySelector("#toDoInput").value.trim());
  toDoItems.push(toDo);
...
// Toddo Event Listener recibe como parámetro el objeto 'event' conteniendo un montón de información que incluye
