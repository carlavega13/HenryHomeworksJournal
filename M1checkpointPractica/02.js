...
const calcularImpuestos = (factura, i = 0) => {
  // Tu código aquí:
  let contador = i;

  if (!Array.isArray(factura)) return "Error";
  if (factura == false && contador > 0) return contador;
  if (factura == false) return false;
  contador += factura.shift();
  return calcularImpuestos(factura, contador);
};
